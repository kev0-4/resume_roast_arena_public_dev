...
# 44. LLM-Judged Content Quality Scoring (2026-09-08)

Direct product feedback after the app had been live for real users for a day: "a lot of okay/medium level resumes are getting 80-90+ ... should the app be more strict? ... relative grading?" Traced the actual scoring code rather than guessing: `compute_score` (`workers/renderer/pipeline/card_data.py`) is a pure deduction from `scored.json`'s rule-engine counts — no relative/percentile grading anywhere, and of the rule engine's issue checks, 11 of 13 are purely structural (does a section exist, is the length in range). Only 2 checks ever assessed actual writing quality (`PASSIVE_VOICE`, `NO_ACTION_VERBS`), and both were gated behind `use_advanced_nlp`, hardcoded `False` with nothing turning it on anywhere in the pipeline — a resume with generic, unquantified, weak-verb bullets paid zero penalty for it, as long as the right sections existed. The LLM roast worker already reads the whole resume and clearly *can* judge this in its prose commentary, but nothing it said ever touched the number.

**Design, agreed with the user before building**: not a second LLM call — one new field (`quality_flags`) on the existing roast-generation call. A small fixed enum of content-quality codes (`GENERIC_BULLETS`, `NO_QUANTIFIED_IMPACT`, `BUZZWORD_FILLER`, `WEAK_ACTION_LANGUAGE`, `SHALLOW_CONTENT`), same spirit as `rules.py`'s codes — the LLM only ever decides which apply, never assigns its own severity (`workers/llm/schemas.py`'s `QUALITY_SEVERITY` owns that mapping), so a flag costs exactly as much as an equivalent-severity structural issue once merged into the score, and results stay auditable rather than the model inventing arbitrary weights per resume. Real cost, measured with Gemini's own `count_tokens` API against the actual production prompt (not guessed): 709 input / 380 output tokens per roast (~$0.0012 at paid rates); the feature adds roughly 100-150 output tokens — a fraction of a cent, and likely still $0 in practice since Gemini's free tier caps by request count, not tokens.

**Structured output, not text-label parsing**: the roast call previously asked Gemini for hand-formatted `VERDICT:`/`ROAST:`/`FIXES:`/`HIGHLIGHTS:` text, regex-parsed by `validator.py` — any formatting slip was an outright parse failure (a real, silently-costly failure mode: burns the call's tokens for nothing, per `processor.py`'s existing `PermanentLLMError` handling). Switched to Gemini's native `response_schema` (JSON mode, `workers/llm/schemas.py`'s `LLMStructuredResponse`) — adding `quality_flags` to a schema is a one-line change, versus another hand-parsed text section, and the model is constrained to the shape directly instead of hoping it follows formatting instructions. `validator.py`'s job shrank to what schema validation *can't* guarantee: non-empty content, and the pre-existing HIGHLIGHTS grounding check (every quote verified as a real verbatim substring of the resume text, hallucinated ones silently dropped) — unchanged in spirit, just operating on parsed fields instead of regex matches.

**Retired rather than turned on**: the dormant `use_advanced_nlp` spaCy path (`workers/normalization/pipeline/signals.py`) and its two rule-engine checks — the LLM's `WEAK_ACTION_LANGUAGE` judges the same thing with real language understanding instead of a hardcoded 21-verb list and a raw dependency-parse count, no reason to run both. `en_core_web_sm` was never even bundled into any worker image, so this path always silently degraded to its dummy fallback in production regardless — not a regression, a real dead branch removed.

**Merge point**: the LLM worker never loaded `scored.json` before this (only a pre-built prompt string), and still doesn't — `roast.json` gains a `quality_issues` field, and the merge into one set of severity counts happens independently in the two places that already load both artifacts: the renderer (`merge_quality_into_summary`, feeds `compute_score`/`compute_stamp`, so the badge baked into the card PNG and the DB's stored `composite_score`/`stamp` reflect it) and the backend's public API (`_merge_quality_into_summary`, a deliberate duplicate — same reason `_compute_stamp` was already duplicated, no shared import boundary between `backend/` and `workers/`). A new "Quality" radar-chart axis (`_compute_subscores`) is fed purely by these codes rather than folded into the existing structural categories, since it's a genuinely different kind of judgment; the frontend's radar chart is fully data-driven (`Object.entries(subscores)`, trig scaled by axis count) so the 7th axis needed zero frontend changes. A cheap deterministic regex (`_quantified_bullet_ratio` in `prompt_builder.py`) counts bullets containing a digit/% and surfaces it as one extra grounding line in the prompt — not a scored rule on its own, just a concrete number for the LLM's own `NO_QUANTIFIED_IMPACT` judgment to react to instead of pure vibes.

**Real bug found during merge-consistency testing, not by inspection**: the backend route recomputed `stamp` fresh from `scored.json`'s *unmerged* summary — meaning the result page's badge could disagree with the badge already baked into its own card image, which the renderer had correctly computed from the *merged* summary. Fixed by merging before `_compute_stamp` in the route too, plus the same merge bug's edge case in both duplicate helpers: `if key in merged` gated the increment on the key already being present in the input dict, which is harmless in production (`ScoreSummary` always has all 5 keys) but a fragile contract — fixed to check against a fixed set of valid severities instead of dict membership, in both copies independently.

**Verified end-to-end against real infra, not just unit tests**: three real Gemini calls (not mocked) through the actual `prompt_builder → client → validator` code path with a strong resume (real metrics, strong verbs), a genuinely mediocre one (real IT-support-tier duties, no invented weakness), and a deliberately weak one — scored 100/68/73 respectively with the rule engine held neutral, the LLM correctly *not* flagging `BUZZWORD_FILLER` on the mediocre resume since it didn't actually use buzzwords (selective judgment, not blanket-flagging). Then a full live pipeline run through all 6 real local workers + real Postgres + real Azurite + a real ingest through the real API: first attempt exposed the stamp-consistency bug above (caught because two pre-existing stale worker processes from earlier in the night — root-owned, no permission to kill or inspect — won the race for the scoring/render stages before this session's own fresh processes could, isolating exactly which stage's code was and wasn't exercised); second attempt, this session's own workers won every stage, and `composite_score` (71), the radar chart's Quality axis (92), and the rendered card's "5 Issues Found" all agreed with each other and with `roast.json`'s actual stored `quality_issues`. Full suite: 146/146 (12 new tests: severity attachment, the merge helpers in both their real locations, the Quality subscore, and the actual "quality issues pull a clean-rule-engine resume's score down" assertion that's the whole point of the feature). Frontend `tsc --noEmit` clean.

**Status**: implemented on `feat/llm-quality-scoring`, not yet pushed/PR'd/merged — built overnight per explicit instruction while the user slept, for their review in the morning.

---

# 45. Next up (queued, not started, 2026-09-15)

Not a full section write-up — the live mock-interview feature (real-time Gemini Live voice interviews, leaderboard, "My Interviews" history, homepage/how-it-works integration) shipped across PRs #21-#26 since section 44 but was never retroactively logged here. This entry exists only to record the backlog agreed with the user for whenever work picks back up, in priority order:

1. **`GET /sessions/me` query design + real production latency investigation.** `/interview/new`'s "pick a roast to defend" card takes 2-3s to populate in production. Traced the route: it does 3-4 sequential DB round trips (user lookup, a COUNT, a paginated SELECT, a stats aggregate the picker doesn't even use) instead of one right-sized, server-side-filtered query. Benchmarked before/after locally against real Postgres (see chat log, 2026-09-15): a right-sized single query (`status=DONE`, no stats) is 66% faster than today's sequential path — but even the *unoptimized* path is sub-2ms on Postgres, which rules out the DB as the dominant cost. The real 2-3s is coming from somewhere else in the chain (Vercel↔Azure network RTT, or Firebase's `verify_id_token` real network call to Google when its cached certs are stale) — next step before writing any fix is a browser Network-tab TTFB check against the live deployment to find out where the time actually goes.
